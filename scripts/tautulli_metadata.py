...
#
# Configuration is read from environment variables (see scripts/.env, not committed):
#   TAUTULLI_URL      e.g. https://tautulli.example.com (defaults to https://tautulli.$DOMAINNAME)
#   TAUTULLI_APIKEY   required
#   PLEX_URL          e.g. http://192.168.1.156:32400 — use the NAS's LAN IP, not localhost/127.0.0.1:
#                     this host's route_localnet sysctl is 0, so loopback connections to Docker's
#                     published ports get dropped as martian packets before reaching the container.
#   PLEX_TOKEN        required
#   DRY_RUN           "false" to actually write to Tautulli; defaults to dry-run (true) for safety
...
TAUTULLI_URL = os.environ.get('TAUTULLI_URL') or (
    'https://tautulli.' + os.environ['DOMAINNAME'] if os.environ.get('DOMAINNAME') else None
)
TAUTULLI_APIKEY = os.environ.get('TAUTULLI_APIKEY')
PLEX_URL = os.environ.get('PLEX_URL')
PLEX_TOKEN = os.environ.get('PLEX_TOKEN')

for _name, _value in (('TAUTULLI_URL', TAUTULLI_URL), ('TAUTULLI_APIKEY', TAUTULLI_APIKEY), ('PLEX_URL', PLEX_URL), ('PLEX_TOKEN', PLEX_TOKEN)):
    if not _value:
        sys.exit("Missing required environment variable: {}".format(_name))
...
DRY_RUN = os.environ.get('DRY_RUN', 'true').lower() != 'false'  # defaults to dry-run; set DRY_RUN=false to write
